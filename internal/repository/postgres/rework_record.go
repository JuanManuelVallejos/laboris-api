...
func (r *ReworkRecordRepository) FindByJobID(jobID string) ([]domain.ReworkRecord, error) {
	rows, err := r.db.Query(context.Background(), `
		SELECT id, job_id, cycle_number, COALESCE(notes,''), quote_amount, no_charge, scheduled_at, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rec.ID, &rec.JobID, &rec.CycleNumber, &rec.Notes, &quoteAmount, &rec.NoCharge, &scheduledAt, &rec.CreatedAt); err != nil {
			return nil, err
		}
...
func (r *ReworkRecordRepository) MarkNoCharge(jobID string, cycleNumber int) error {
	_, err := r.db.Exec(context.Background(), `
		UPDATE job_rework_records SET no_charge = true
		WHERE job_id = $1 AND cycle_number = $2
	`, jobID, cycleNumber)
	return err
}
